        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div className="kpi-card" style={{ padding: 32 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                ...
                <button className="btn-gold-sm" onClick={() => navigate('/porteur/nouveau')}>✎ Modifier</button>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                {[
                  ...
                  <div key={s.label} style={{ padding: 14, background: 'var(--dark-4)', border: '1px solid var(--border)' }}>
                    ...
                    <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 18, color: 'var(--gold-light)' }}>{s.value}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div className="kpi-card" style={{ padding: 24 }}>
              ...
              {[
                { label: 'Vues totales', value: selected?.views ?? 0, icon: '◷' },
                { label: 'En favoris', value: selected?.favorites_count ?? 0, icon: '♦' },
                { label: 'Investissements', value: selected?.investment_count ?? 0, icon: '◎' },
                { label: 'Risque', value: selected?.risk_level ?? '—', icon: '⚡' },
              ].map(s => (
                <div key={s.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <span style={{ color: 'var(--gold)', fontSize: 14 }}>{s.icon}</span>
